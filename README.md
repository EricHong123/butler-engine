...
docker compose -f docker-compose.prod.yml exec backend python infra/scripts/seed_demo.py
```

打开 http://localhost，登录 PIN：`888888`（洪伟）/ `123456`（管理员）

### 方式二：手动启动

**环境要求**：Python 3.12+ · Node.js 18+ · uv
...
```
浏览器 :80 → Nginx → Frontend (Next.js :3000) ─(rewrites)─→ Backend (FastAPI :8000)
                                          ↓
                                    企业微信 Webhook → /wechat/* → Backend
                                                                    ↓
...
| 服务 | 镜像 | 端口 | 说明 |
|------|------|------|------|
| `nginx` | nginx:alpine | 80 | 反向代理入口 |
| `frontend` | infra-frontend | 3000 | Next.js 14 SSR + API 代理 |
| `backend` | infra-backend | 8000 | FastAPI Agent 引擎 |
| `db` | postgres:16-alpine | 5432 | 主数据库 |
| `redis` | redis:7-alpine | 6379 | 审核队列 / 缓存 |
...
在 `infra/.env` 中修改 `BUTLER_PROVIDER` 即可，支持 13 家供应商。支持列表参见上方 [LLM 供应商配置](#llm-供应商配置)。
